...
import css from './reactFastMarquee.module.css'
...
const Marquee: FC<MarqueeProps> = ({
      ...
      return [...Array(Number.isFinite(multiplier) && multiplier >= 0 ? multiplier : 0)].map((_, i) => (
        <Fragment key={i}>
          {Children.map(children, (child) => {
            return (
              <div style={childStyle} className={css.rfmChild}>
                {child}
              </div>
            )
          })}
  ...
  return !isMounted ? null : (
    <div ref={containerRef} style={containerStyle} className={`${css.rfmMarqueeContainer} ${className}`}>
      {gradient && <div style={gradientStyle} className={css.rfmOverlay} />}
      <div
        className={css.rfmMarquee}
        style={marqueeStyle}
        onAnimationIteration={onCycleComplete}
        onAnimationEnd={onFinish}
      >
        <div className={css.rfmInitialChildContainer} ref={marqueeRef}>
          {Children.map(children, (child) => {
            return (
              <div style={childStyle} className={css.rfmChild}>
                {child}
      ...
      <div className={css.rfmMarquee} style={marqueeStyle}>
